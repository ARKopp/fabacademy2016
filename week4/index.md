...
The FabISP is an in-system programmer vor AVR microrontollers. It allows you to programm the microcontrollers on other board you make during Fabacademy. Normally this thing would cost you like 30€ or you would need to build one with an arduino. 
...
Also I send the board to a PCB (Printed Circuit Board) house. 
...
* Transparent Foil 
* [Entwickler (Natriumperoxid) ](http://www.reichelt.de/?GRUPPE=C95)
...
## 4. Machining with iModela 

I borrowed a iModela but did not have the time yet to use it but I will give it a try soon. 


http://garage-lab.de/allgemein/der-japanische-neuzugang-roland-imodela/

1/64 traces (innen)
1/32 interior (umriss)
...
Before plugin in the board into the computer I checked if my solder joints where correct by checking continuity. The need to be nice and shiny. If not you should reflow them. 

I then checked if there is no short on the power line and Ground and Power are not conntected. On the left side I saw that ther was a little connection and I got rid of it. 

Also I made a "Smoke Test" for which you plugin in the Board with the USB Cable into your computer and if you get any error messages from your computer I would have a short somewhere on my board. 
...
Then I followed [this steps](http://archive.fabacademy.org/archives/2016/doc/programming_FabISP.html) to program the chip to become a FabISP. I used a DIAMEX-Prog S but I late saw that I have to install some drivers so I am waiting for my [AVR ISP from Atmel.](http://www.amazon.de/Programmer-USB-AVRISP-XPII-Compatible/dp/B00KM6ZA9I?ie=UTF8&psc=1&redirect=true&ref_=oh_aui_detailpage_o02_s00).
...
Then I tried it with another programmer a [Programmierer-Adapter 3.3V oder 5V USBASP Download-Programm ISP-AVR](http://www.amazon.de/Programmierer-Adapter-3-3V-USBASP-Download-Programm-ISP-AVR/dp/B00AVRHVPO/ref=pd_sim_147_2?ie=UTF8&dpID=41Z%2BPxyWweL&dpSrc=sims&preST=_AC_UL160_SR160%2C160_&refRID=0C24998P250D0C4VJZFP). There I realized that you can change between 3V and 5V. Still I got a red light and I have to got back to check for shorts. 
